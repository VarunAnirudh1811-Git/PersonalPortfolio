...
if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS || !process.env.EMAIL_TO) {
  console.error('ERROR: Missing required environment variables (EMAIL_USER, EMAIL_PASS, EMAIL_TO)')
  process.exit(1)
}
...
const contactLimiter = rateLimit({
  ...
  standardHeaders: true, // enable `RateLimit-*` headers for client feedback
...
app.post('/api/contact', contactLimiter, function(req, res) {
  ...
  // Accept emails with + in local part (e.g., user+tag@example.com)
